...
arguments. `<number>` shall be an `Integer`, `Long`, `Single`, `Currency`,
`Decimal`, or `Double`; the optional `<digits>` shall be a `Long` naming the
number of decimal places to retain.

`Single`/`Currency`/`Decimal`/`Double` values shall use banker's rounding
(round half to even). A `Long` or `Integer` value is already whole and
remains unchanged. `Round` preserves a `Single`, `Currency`, `Decimal`, or
`Double` argument's type in the result. `Currency` and `Decimal` round using exact
...
in `REQ-0196`, exact `Decimal` preservation in `REQ-0198`, and `Integer`
in `REQ-0199`. `Single`/`Double` decimal-place rounding follows the host
IEEE-754 representation; `Currency`/`Decimal` rounding is exact base-10
fixed-point arithmetic, not a floating-point approximation.
...
- `tests/evaluator_tests.cpp` covers one- and two-argument calls, positive and
  ...
  count, preserved result type across `Long`/`Integer`/`Single`/`Currency`/
  `Decimal`/`Double`, wrong arity, a non-numeric argument, and a negative
  digit count.
