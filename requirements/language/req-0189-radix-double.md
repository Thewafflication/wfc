...
`Decimal`, `Double`, or `String` containing a complete finite
decimal/exponent number after surrounding ASCII whitespace is removed. A
`Single`, `Currency`, `Decimal`, `Double`, or numeric `String` shall first be
rounded to the nearest even `Long` (banker's rounding). The resulting 32-bit
value shall then be rendered using the existing uppercase two's-complement
...
`Single` inputs followed in `REQ-0195`, `Currency` inputs in `REQ-0196`, and
`Decimal` inputs in `REQ-0198`. This requirement retains the current 32-bit
`Long` radix width.

## Verification

- `tests/evaluator_tests.cpp` covers half-even rounding in both directions,
  `Single`, `Currency`, and `Decimal` inputs, numeric Strings with whitespace
  and exponent forms, negative two's-complement output, malformed/non-numeric
