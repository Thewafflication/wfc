...
- `Format`'s `%`/`E+`/`E-` combined with a custom picture, a `\` escape
  character, a fourth (text) custom-picture section, and the `Date`/
  `Time` named styles listed in `REQ-0193`'s Scope (`Currency`, the core
  custom-picture tokens, and up to three `;`-separated numeric sections
  are now implemented, `Currency` under a disclosed fixed-locale
  rendering);
