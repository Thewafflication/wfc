...
- `Format`'s `%`/`E+`/`E-` combined with a custom picture, multiple
  `;`-separated sections, a `\` escape character, and the `Date`/`Time`
  named styles listed in `REQ-0193`'s Scope (`Currency` and the core
  custom-picture tokens are now implemented, the former under a disclosed
