...
- `Format`'s `%`/`E+`/`E-` combined with a custom picture, quote-
  delimited (`"..."`) literal text, a fourth (text) custom-picture
  section, and the `Date`/`Time` named styles listed in `REQ-0193`'s
  Scope (`Currency`, the core custom-picture tokens, up to three
  `;`-separated numeric sections, and a `\` escape character are now
  implemented, `Currency` under a disclosed fixed-locale rendering);
