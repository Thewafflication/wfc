...
- `Format`'s `%`/`E+`/`E-` combined with a custom picture, a fourth
  (text) custom-picture section, and the `Date`/`Time` named styles
  listed in `REQ-0193`'s Scope (`Currency`, the core custom-picture
  tokens, up to three `;`-separated numeric sections, and both literal-
  text mechanisms -- `\` escape and `"..."` quoting -- are now
