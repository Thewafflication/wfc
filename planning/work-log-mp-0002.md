...
- dynamic multi-dimensional arrays (`Dim arr(,) As Type` / `ReDim` changing
  ...
  fixed-size only; array-typed `Sub`/`Function` parameters, and
  `Variant`/`Object`-element (dynamic or fixed, 1-D or multi-dim) arrays --
  the two largest items from the owner's original pasted list, still
  deliberately excluded from every array requirement's scope so far;
